...
plugin manager, then restart the app. That's it — the nine skills below are ready
to use.
...
├── usage.md      which units recall actually draws on (what earns its keep)
├── friction.md   what deployed workflow skills hit that their unit didn't cover
...
├── skills/       workflow skills generated by scaffold (+ its manifest)
└── firm-plugin/  the shareable plugin bundling the firm's scaffolded skills
```
...
Deployed workflows share the same way: scaffold collects the firm's generated
skills into `firm-plugin/` inside the knowledge folder — a normal Claude
plugin the rest of the team installs **once** (zip upload in Cowork / Desktop,
or `claude plugin marketplace add` on the folder in the CLI) and updates when
scaffold regenerates something. `walled` / `client` units never go in it.
...
| `recall` | "how do we …", "what's our process for …" | Searches your captured files **first**, answers from the firm's own units (never generic advice as firm policy), respects draft status + confidentiality. Logs hits to `usage.md` and misses to `gaps.md`, so the firm learns what's used and what's missing. |
...
| `landscape` | "where are the gaps", "do any of these conflict", "audit the knowledge base" | Reads all units together and reports gaps, overlaps, contradictions, and disconnects — diffed against practice-area coverage baselines and weighted by real usage (`usage.md`, `friction.md`, `gaps.md`). Read-only audit; recommends follow-ups but changes nothing. |
| `improve` | "improve our X process", "critique this procedure", "can this be streamlined" | Critiques a **promoted** unit — grounded in logged friction, gaps, and usage before general opinion — and, on your say-so, writes the proposed revision as a new draft that `supersedes` the original. The original stays firm procedure until a human promotes the proposal in review. |
...
| `scaffold` | "turn this into a skill", "make this a workflow", "deploy this process", "are my skills up to date" | Turns a **promoted** unit into a runnable `SKILL.md` you can invoke as a workflow — the inverse of capture. Installs per-user or into the shared **firm plugin** (one artifact the whole team installs); generated skills report uncovered situations back to `friction.md`. `--check` finds scaffolded skills whose source unit was re-promoted and regenerates them. |

Skills trigger on natural phrasing, or invoke explicitly: `/koan:intro`,
`/koan:recall`, `/koan:capture`, `/koan:ingest`, `/koan:review`,
`/koan:landscape`, `/koan:map`, `/koan:improve`, `/koan:scaffold`.

## The lifecycle

**Capture → review → recall**, then **improve** and **deploy** — and the loop
feeds itself:

1. **Capture** turns one "how we do X" into a **draft** in `inbox/`. Three
   doors in, cheapest first: **ingest** a document the firm already has (its
   text goes to `docs/`, its procedures become drafts), **session capture**
   ("capture what we just did") distills a draft from work just done with
   Claude, or the **debrief** interviews it out of someone's head.
...
3. **Recall** answers "how do we …" questions from the promoted units, attributed
   to the source, never invented. Every answer logs a hit to `usage.md`; every
   miss logs the question to `gaps.md` — so the firm knows what's actually used
   and what people needed and didn't get.
4. **Improve** critiques a promoted unit — grounded in the logged friction,
   gaps, and usage — and proposes a revision as a new draft that supersedes the
   original through the same review gate. Nothing changes until a human
   promotes it.
5. **Scaffold** turns a promoted unit into a runnable workflow skill, installed
   per-user or into the shared **firm plugin**. Deployed skills log situations
   their steps didn't cover to `friction.md` — which is exactly what `improve`
   and `landscape` read next time around.
